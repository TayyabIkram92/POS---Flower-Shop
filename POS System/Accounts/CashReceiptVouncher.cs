...
{
    public partial class CashReceiptVouncher : Form
    {
        SqlConnection con = new SqlConnection();
        public CashReceiptVouncher()
        {
            InitializeComponent();
        ...
        public void Load_Data()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("Select VouncherNo from CashReceiptVounchers", con);
            ...
            if (flag == true)
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("Select max(VouncherNo) from CashReceiptVounchers", con);
                    SqlDataReader rd = cmd.ExecuteReader();
                    if (rd.Read())
                    {
                        ...
                        num++;
                        txtsaleno.Text = "CRV-" + num.ToString();
                    }
                    con.Close();
                }
                catch (Exception exp)
            ...
            {
                txtsaleno.Text = "CRV-10000001";
            }
        }
        private void CashReceiptVouncher_Load(object sender, EventArgs e)
        {
            Load_Data();
        ...
        private void txtcn_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dr.Read())
                {
                    ...
                    dataGridView2.Rows.Add(txtcn.Text, dr[2], "0", "Cash Received");
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if (textBox4.Text == string.Empty)
            {
                if (dataGridView2.Rows.Count > 0)
                {
                    ...
                    {
                        dataGridView2.Rows[0].Cells[3].Value = "Cash Received";
                    }
                    try
                    {
                        con.Open();
                        SqlCommand cmd = new SqlCommand("Insert into CashReceiptVounchers values(@a,@b,@c,@d,@e,@f,@g,@h,@i,@j,@k)", con);
                    ...
                    try
                    {
                        ...
                        cmd.ExecuteNonQuery();
                        con.Close();
                        MessageBox.Show("Cash Receipt Vouncher Saved Successfully.");
